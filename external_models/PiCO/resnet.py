...
model_dict = {
    ...
    'wideresnet': [wideresnet, 128],
    'presnet': [PreResNet, 512],
}
...
class SupConResNet(nn.Module):
    ...
    def __init__(self, args, name='resnet18', head='mlp', feat_dim=128, num_class=0, pretrained=False):
        super(SupConResNet, self).__init__()
        model_fun, dim_in = model_dict[name]
        # dim_in = 128 if args.dataset == 'cifar10' else 512
        # dim_in = 128
        if pretrained:
            ...
            if name == 'resnet18':
                ...
                model = wideresnet.build_wideresnet_nofc(depth=args.model_depth,
                                                         widen_factor=args.model_width,
                                                         dropout=0,
                                                         num_classes=args.num_classes)
                self.encoder = model
            elif name == 'presnet':
                model = PreResNet.PreResNetNoFC18(num_class=args.num_classes)
                self.encoder = model
