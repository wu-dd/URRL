...
class PreResNet(nn.Module):
    ...
    def forward(self, x, lin=0, lout=5, do_recon=False, has_out=True, has_feat=False):
        out = x
        if lin < 1 and lout > -1:
            out = self.conv1(out)
            out = self.bn1(out)
...
def PreResNetNoFC18(num_class=10, low_dim=20):
    return PreResNetNoFC(PreActBlock, [2,2,2,2], num_classes=num_class, low_dim=low_dim)
