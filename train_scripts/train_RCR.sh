log_path='/mnt/nfs-storage-personal/log/RCR/RCR_eid'
loss='RCR'
...
for eid in `seq $e_s $e_e`;do
    python train_RCR.py -eid ${eid} -loss $loss -dataset cifar10 -partial_rate $pr -noisy_rate $nr -exclusion none -seed `expr ${eid} - ${e_s}` > $log_path$eid".log" 2>&1
done
...
e_e=105
for eid in `seq $e_s $e_e`;do
    python train_RCR.py -eid ${eid} -loss $loss -dataset cifar100 -partial_rate $pr -noisy_rate $nr -exclusion none -seed `expr ${eid} - ${e_s}` > $log_path$eid".log" 2>&1
done
